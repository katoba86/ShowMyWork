...
class Sticky {
    constructor (el, vm) {
        ...
        const keep = this.getAttribute('sticky-keep') || null;
        const keepOffset = this.getAttribute('sticky-keep-offset') || 0;
        const keepOffsetMultiply = this.getAttribute('sticky-keep-multiply') || 0;
        ...
        this.keep = this.el.querySelector('.'+keep);
        ...
        this.options = {
            ...
            keepOffsetMultiply:keepOffsetMultiply,
            keepIndex:0,
        ...
        vm.$eventBus.$on('send-data', (data) => {
            this.options.keepIndex = parseInt(data.index)-1;
            this.updateElements();
        });
    }
    ...
    updateKeep(elStyle){
        let elTop = (isNaN(parseInt(elStyle.top)))?0:parseInt(elStyle.top);
        let ko = (isNaN(parseInt(this.options.keepOffset)))?0:parseInt(this.options.keepOffset);
        let m = (isNaN(parseInt((this.options.keepIndex*this.options.keepOffsetMultiply))))?0:parseInt((this.options.keepIndex*this.options.keepOffsetMultiply));


        batchStyle(this.keep, {top:elTop+ko+m+"px"}, {});
    }
